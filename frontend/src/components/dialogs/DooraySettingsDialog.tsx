  ...
  ({ projectId }) => {
    ...
    useEffect(() => {
      ...
      if (isConnected && settings) {
        // If connected but no project selected, show project selection
        if (!settings.selected_project_id) {
          setStep('project');
        } else {
          setStep('connected');
          // Initialize selected project ID from settings
          setSelectedProjectId(settings.selected_project_id);
        }
    ...
    const renderTokenStep = () => (
      <>
        <DialogHeader>
          ...
          <DialogDescription>
            Dooray 도메인과 API 토큰을 입력하여 프로젝트와 연동합니다.
          </DialogDescription>
        </DialogHeader>
        <DialogContent>
          <div className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="dooray-domain">Dooray 도메인</Label>
              <Input
                id="dooray-domain"
                type="text"
                placeholder="example.dooray.com"
                value={domain}
                onChange={(e) => setDomain(e.target.value)}
              />
              <p className="text-xs text-muted-foreground">
                조직의 Dooray 도메인을 입력하세요 (예: nhnent.dooray.com).
              </p>
            </div>
            <div className="space-y-2">
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={!token.trim() || !domain.trim() || isSaving}
          >
            {isSaving ? (
              <>
    ...
    const renderProjectStep = () => (
      <>
        <DialogHeader>
          <DialogTitle>Dooray 프로젝트 선택</DialogTitle>
          <DialogDescription>
            연동할 Dooray 프로젝트를 선택하세요.
          </DialogDescription>
        </DialogHeader>
        <DialogContent>
          <div className="space-y-4">
            {isLoadingProjects ? (
              ...
              <div className="space-y-2">
                <Label htmlFor="dooray-project">프로젝트</Label>
                <select
                  id="dooray-project"
        ...
        <DialogFooter>
          <Button variant="outline" onClick={handleDisconnect} disabled={isDeleting}>
            {isDeleting ? (
              <>
                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                해제 중...
    ...
    const renderConnectedStep = () => (
      <>
        <DialogHeader>
          ...
          <DialogDescription>
            {settings?.selected_project_name || '프로젝트'} 프로젝트와 연동되었습니다.
          </DialogDescription>
        </DialogHeader>
        <DialogContent>
          <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="mt-2 text-sm text-muted-foreground">
                    프로젝트: {settings?.selected_project_name || '선택되지 않음'}
                  </div>
                </div>
                <Button
                  variant="ghost"
