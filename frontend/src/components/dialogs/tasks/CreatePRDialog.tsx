...
const CreatePRDialogImpl = NiceModal.create<CreatePRDialogProps>(
  ({ attempt, task, repoId, targetBranch }) => {
    ...
    const [autoGenerateDescription, setAutoGenerateDescription] = useState(
      config?.pr_auto_description_enabled ?? true
    );
    ...
    // Load AI preview when autoGenerateDescription is toggled on
    const loadPreview = useCallback(async () => {
      ...
      try {
        const preview = await attemptsApi.previewPRDescription(attempt.id, repoId);
        setPrTitle(preview.title);
        setPrBody(preview.body);
        setPreviewLoaded(true);
      } catch (err) {
        setError(
          err instanceof Error
            ? err.message
            : t('createPrDialog.errors.previewFailed', 'Failed to generate preview')
        );
      } finally {
    ...
    // Auto-load preview when checkbox is checked
    useEffect(() => {
      if (autoGenerateDescription && modal.visible && !previewLoaded && !isLoadingPreview) {
        loadPreview();
      }
    }, [autoGenerateDescription, modal.visible, previewLoaded, isLoadingPreview, loadPreview]);
    ...
    const handleConfirmCreatePR = useCallback(async () => {
      ...
      // If preview was already loaded, don't trigger AI generation again
      // (user may have edited the preview)
      const shouldAutoGenerate = autoGenerateDescription && !previewLoaded;
      ...
      const result = await attemptsApi.createPR(attempt.id, {
        ...
        draft: isDraft,
        auto_generate_description: shouldAutoGenerate,
        repo_id: repoId,
      });
          ...
          <DialogContent className="sm:max-w-[525px]">
                ...
                <div className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => {
                      setAutoGenerateDescription(checked as boolean);
                      if (!checked) {
                        // Reset preview state when unchecked
                        setPreviewLoaded(false);
                      }
                    }}
                  ...
                  {previewLoaded && !isLoadingPreview && (
                    <span className="text-xs text-green-600">
                      {t('createPrDialog.previewLoaded', 'Preview loaded - you can edit')}
                    </span>
                  )}
                </div>
            ...
            <DialogFooter>
              ...
              <Button
                ...
                disabled={creatingPR || isLoadingPreview || !prTitle.trim()}
