...
impl GhCli {
    ...
    pub fn create_pr(
        ...
        // For GitHub Enterprise, include the host in the repo specifier
        let repo_specifier = match host {
            Some(h) => format!("{}/{}/{}", h, owner, repo_name),
            None => format!("{}/{}", owner, repo_name),
        };
        ...
        args.push(OsString::from(repo_specifier));
    ...
    pub fn get_pr_review_comments(
        ...
        // For GitHub Enterprise, specify the hostname
        if let Some(h) = host {
            args.push("--hostname".to_string());
            args.push(h.to_string());
        }
        let raw = self.run(args.iter().map(|s| s.as_str()), None)?;
